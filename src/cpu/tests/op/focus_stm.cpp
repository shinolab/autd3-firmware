...
TEST(Op, FocusSTM) {
  ...
  for (uint64_t i = 0; i < 65536; i++) buf.push_back(i << 48 | i << 32 | i << 16 | i);
  ...
  while (cnt < 65536) {
    ...
    auto send = std::min(65536 - cnt, (sizeof(RX_STR) - sizeof(Header) - offset) / 8);
  ...
  ASSERT_EQ(bram_read_raw(BRAM_SELECT_CONTROLLER, BRAM_ADDR_CTL_FLAG) & CTL_FLAG_STM_GAIN_MODE, 0);
  ...
  ASSERT_EQ(bram_read_raw(BRAM_SELECT_CONTROLLER, BRAM_ADDR_SOUND_SPEED_0), 0xDEF0);
  ASSERT_EQ(bram_read_raw(BRAM_SELECT_CONTROLLER, BRAM_ADDR_SOUND_SPEED_1), 0x9ABC);
  ...
  for (size_t i = 0; i < 65536; i++) {
    ...
    ASSERT_EQ(bram_read_raw(BRAM_SELECT_STM, 8 * i + 1), (buf[i] >> 16) & 0xFFFF);
    ASSERT_EQ(bram_read_raw(BRAM_SELECT_STM, 8 * i + 2), (buf[i] >> 32) & 0xFFFF);
    ASSERT_EQ(bram_read_raw(BRAM_SELECT_STM, 8 * i + 3), (buf[i] >> 48) & 0xFFFF);
  }
}
...
TEST(Op, FocusSTMWithStartFinishIdx) {
  ...
  while (cnt < 65536) {
    ...
    auto offset = 4;
    if (cnt == 0) {
      data_body[1] = FOCUS_STM_FLAG_BEGIN | FOCUS_STM_FLAG_USE_START_IDX | FOCUS_STM_FLAG_USE_FINISH_IDX;
      *reinterpret_cast<uint32_t*>(data_body + 4) = freq_div;
      *reinterpret_cast<uint32_t*>(data_body + 8) = sound_speed;
