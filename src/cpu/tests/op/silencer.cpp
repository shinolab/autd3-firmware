...
TEST(Op, SilencerFixedUpdateRate) {
  ...
  ASSERT_EQ(bram_read_controller(ADDR_SILENCER_UPDATE_RATE_INTENSITY), intensity << 8);
  ASSERT_EQ(bram_read_controller(ADDR_SILENCER_UPDATE_RATE_PHASE), phase << 8);
...
TEST(Op, SilencerFixedUpdateRate2) {
  init_app();
  ...
  const auto p = reinterpret_cast<uint8_t*>(data.data) + sizeof(Header);
  reinterpret_cast<ConfigSilencer2*>(p)->tag = TAG_SILENCER2;
  reinterpret_cast<ConfigSilencer2*>(p)->flag = flag;
  reinterpret_cast<ConfigSilencer2*>(p)->value_intensity = intensity;
  reinterpret_cast<ConfigSilencer2*>(p)->value_phase = phase;
...
TEST(Op, SilencerFixedCompletionSteps2) {
  init_app();
  ...
  ASSERT_EQ(bram_read_controller(ADDR_SILENCER_COMPLETION_STEPS_INTENSITY), intensity & 0xFF);
  ASSERT_EQ(bram_read_controller(ADDR_SILENCER_COMPLETION_STEPS_PHASE), phase & 0xFF);
  ASSERT_EQ(bram_read_controller(ADDR_SILENCER_FLAG), flag);
}
TEST(Op, SilencerFixedCompletionStepsInvaidStepMod2) {
  init_app();
...
TEST(Op, SilencerFixedCompletionStepsInvaidStepSTM2) {
  init_app();
