...
TEST(Op, SilencerFixedUpdateRate) {
  ...
  ASSERT_EQ(bram_read_controller(ADDR_SILENCER_MODE), SILENCER_MODE_FIXED_UPDATE_RATE);
}
...
TEST(Op, SilencerFixedCompletionSteps) {
  ...
  const auto flag = SILENCER_MODE_FIXED_COMPLETION_STEPS | SILENCER_FLAG_STRICT_MODE;
  ...
  ASSERT_EQ(bram_read_controller(ADDR_SILENCER_MODE), SILENCER_MODE_FIXED_COMPLETION_STEPS);
}
