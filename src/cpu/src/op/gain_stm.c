...
uint8_t write_gain_stm(const volatile uint8_t* p_data) {
  ...
  static_assert(offsetof(GainSTMHead, transition_mode) == 3, "GainSTM is not valid.");
  ...
  static_assert(offsetof(GainSTMHead, transition_value) == 8, "GainSTM is not valid.");
  ...
  if ((p->subseq.flag & GAIN_STM_FLAG_END) == GAIN_STM_FLAG_END) {
    ...
    if ((p->subseq.flag & GAIN_STM_FLAG_UPDATE) != 0) return stm_segment_update(segment, _stm_transition_mode, _stm_transition_value);
  }
...
uint8_t change_gain_stm_segment(const volatile uint8_t* p_data) {
  ...
  static_assert(offsetof(GainSTMUpdate, transition_mode) == 2, "GainSTM is not valid.");
  static_assert(offsetof(GainSTMUpdate, transition_value) == 8, "GainSTM is not valid.");
