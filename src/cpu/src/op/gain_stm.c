...
uint8_t write_gain_stm(const volatile uint8_t* p_data) {
  ...
  static_assert(offsetof(GainSTMHead, size) == 2, "GainSTM is not valid.");
  static_assert(offsetof(GainSTMHead, mode) == 3, "GainSTM is not valid.");
  ...
  static_assert(offsetof(GainSTMHead, transition) == 8, "GainSTM is not valid.");
  static_assert(sizeof(GainSTMSubseq) == 4, "GainSTM is not valid.");
  ...
  static_assert(offsetof(GainSTMSubseq, size) == 2, "GainSTM is not valid.");
  ...
  const uint8_t send = p->subseq.size;
  ...
  if ((p->subseq.flag & GAIN_STM_FLAG_BEGIN) == GAIN_STM_FLAG_BEGIN) {
    if (validate_transition_mode(_stm_segment, segment, p->head.rep, p->head.transition.MODE.mode)) return ERR_INVALID_TRANSITION_MODE;

    if (validate_silencer_settings(p->head.freq_div, _mod_freq_div[_mod_segment])) return ERR_INVALID_SILENCER_SETTING;

    if (p->head.transition.MODE.mode != TRANSITION_MODE_NONE) _stm_segment = segment;
    _stm_cycle[segment] = 0;
    _gain_stm_mode = p->head.mode;
    _stm_transition = p->head.transition;
    _stm_freq_div[segment] = p->head.freq_div;
    _stm_rep[segment] = p->head.rep;

    switch (segment) {
      case 0:
        bram_write(BRAM_SELECT_CONTROLLER, ADDR_STM_FREQ_DIV0, _stm_freq_div[segment]);
        bram_write(BRAM_SELECT_CONTROLLER, ADDR_STM_MODE0, STM_MODE_GAIN);
        bram_write(BRAM_SELECT_CONTROLLER, ADDR_STM_REP0, p->head.rep);
        break;
      case 1:
        bram_write(BRAM_SELECT_CONTROLLER, ADDR_STM_FREQ_DIV1, _stm_freq_div[segment]);
  ...
  switch (_gain_stm_mode) {
    case GAIN_STM_MODE_INTENSITY_PHASE_FULL:
      bram_cpy_volatile(BRAM_SELECT_STM, (_stm_cycle[segment] & GAIN_STM_BUF_PAGE_SIZE_MASK) << 8, src, NUM_TRANSDUCERS);
      _stm_cycle[segment] = _stm_cycle[segment] + 1;
      break;
    case GAIN_STM_MODE_PHASE_FULL:
      bram_cpy_gain_stm_phase_full((_stm_cycle[segment] & GAIN_STM_BUF_PAGE_SIZE_MASK) << 8, src, 0, NUM_TRANSDUCERS);
      _stm_cycle[segment] = _stm_cycle[segment] + 1;

      if (send > 1) {
        src = src_base;
        bram_cpy_gain_stm_phase_full((_stm_cycle[segment] & GAIN_STM_BUF_PAGE_SIZE_MASK) << 8, src, 8, NUM_TRANSDUCERS);
        _stm_cycle[segment] = _stm_cycle[segment] + 1;
      }
      break;
    case GAIN_STM_MODE_PHASE_HALF:
      bram_cpy_gain_stm_phase_half((_stm_cycle[segment] & GAIN_STM_BUF_PAGE_SIZE_MASK) << 8, src, 0, NUM_TRANSDUCERS);
      _stm_cycle[segment] = _stm_cycle[segment] + 1;

      if (send > 1) {
        src = src_base;
        bram_cpy_gain_stm_phase_half((_stm_cycle[segment] & GAIN_STM_BUF_PAGE_SIZE_MASK) << 8, src, 4, NUM_TRANSDUCERS);
        _stm_cycle[segment] = _stm_cycle[segment] + 1;
      }

      if (send > 2) {
        src = src_base;
        bram_cpy_gain_stm_phase_half((_stm_cycle[segment] & GAIN_STM_BUF_PAGE_SIZE_MASK) << 8, src, 8, NUM_TRANSDUCERS);
        _stm_cycle[segment] = _stm_cycle[segment] + 1;
      }

      if (send > 3) {
        src = src_base;
        bram_cpy_gain_stm_phase_half((_stm_cycle[segment] & GAIN_STM_BUF_PAGE_SIZE_MASK) << 8, src, 12, NUM_TRANSDUCERS);
        _stm_cycle[segment] = _stm_cycle[segment] + 1;
      }
      break;
    default:
  ...
  if ((_stm_cycle[segment] & GAIN_STM_BUF_PAGE_SIZE_MASK) == 0)
    change_stm_wr_page((_stm_cycle[segment] & ~GAIN_STM_BUF_PAGE_SIZE_MASK) >> GAIN_STM_BUF_PAGE_SIZE_WIDTH);
  ...
  if ((p->subseq.flag & GAIN_STM_FLAG_END) == GAIN_STM_FLAG_END) {
    ...
    if ((p->subseq.flag & GAIN_STM_FLAG_UPDATE) != 0) return stm_segment_update(segment, _stm_transition);
  }
...
uint8_t change_gain_stm_segment(const volatile uint8_t* p_data) {
  ...
  static_assert(offsetof(GainSTMUpdate, transition) == 8, "GainSTM is not valid.");
  ...
  if (validate_transition_mode(_stm_segment, p->segment, _stm_rep[p->segment], p->transition.MODE.mode)) return ERR_INVALID_TRANSITION_MODE;
  ...
  return stm_segment_update(p->segment, p->transition);
}
