...
uint8_t clear(void) {
  ...
  int i;
  for (i = 0; i < 255; i++) bram_write(BRAM_SELECT_PWE_TABLE, i, (const uint16_t)asin_table[i]);
  bram_write(BRAM_SELECT_PWE_TABLE, i, 0x0100);

  bram_set(BRAM_SELECT_CONTROLLER, BRAM_CNT_SELECT_PHASE_CORR << 8, 0x0000, ((NUM_TRANSDUCERS + 1) >> 1));

  bram_set(BRAM_SELECT_CONTROLLER, BRAM_CNT_SELECT_OUTPUT_MASK << 8, 0xFFFF, 32);
