...
uint8_t write_foci_stm(const volatile uint8_t* p_data) {
  ...
  static_assert(offsetof(FocusSTMHead, transition_mode) == 4,
                "FocusSTM is not valid.");
  static_assert(offsetof(FocusSTMHead, num_foci) == 5,
                "FocusSTM is not valid.");
  static_assert(offsetof(FocusSTMHead, sound_speed) == 6,
                "FocusSTM is not valid.");
  static_assert(offsetof(FocusSTMHead, freq_div) == 8,
                "FocusSTM is not valid.");
  ...
  if ((p->subseq.flag & FOCUS_STM_FLAG_BEGIN) == FOCUS_STM_FLAG_BEGIN) {
    ...
    switch (segment) {
      case 0:
        ...
        bram_write(BRAM_SELECT_CONTROLLER, ADDR_STM_SOUND_SPEED0,
                   p->head.sound_speed);
      ...
      case 1:
        ...
        bram_write(BRAM_SELECT_CONTROLLER, ADDR_STM_SOUND_SPEED1,
                   p->head.sound_speed);
  ...
  page_capacity = (_stm_cycle[segment] & ~FOCUS_STM_BUF_PAGE_SIZE_MASK) +
                  FOCUS_STM_BUF_PAGE_SIZE - _stm_cycle[segment];
  if (size < page_capacity) {
    bram_cpy_focus_stm(
        (_stm_cycle[segment] & FOCUS_STM_BUF_PAGE_SIZE_MASK) << 5, src, size,
        _num_foci);
    _stm_cycle[segment] = _stm_cycle[segment] + size;
  } else {
    bram_cpy_focus_stm(
        (_stm_cycle[segment] & FOCUS_STM_BUF_PAGE_SIZE_MASK) << 5, src,
        page_capacity, _num_foci);
    _stm_cycle[segment] = _stm_cycle[segment] + page_capacity;

    change_stm_wr_page((_stm_cycle[segment] & ~FOCUS_STM_BUF_PAGE_SIZE_MASK) >>
                       FOCUS_STM_BUF_PAGE_SIZE_WIDTH);

    bram_cpy_focus_stm(
        (_stm_cycle[segment] & FOCUS_STM_BUF_PAGE_SIZE_MASK) << 5,
        src + 4 * page_capacity * _num_foci, size - page_capacity, _num_foci);
