...
extern Transition _stm_transition;
...
uint8_t write_foci_stm(const volatile uint8_t* p_data) {
  static_assert(sizeof(Transition) == 8, "FocusSTM is not valid.");
  ...
  static_assert(offsetof(FocusSTMHead, transition) == 16, "FocusSTM is not valid.");
  ...
  if ((p->subseq.flag & FOCUS_STM_FLAG_END) == FOCUS_STM_FLAG_END) {
    ...
    if ((p->subseq.flag & FOCUS_STM_FLAG_UPDATE) != 0) return stm_segment_update(segment, _stm_transition);
  }
...
uint8_t change_foci_stm_segment(const volatile uint8_t* p_data) {
  ...
  static_assert(offsetof(FocusSTMUpdate, transition) == 8, "FocusSTM is not valid.");
