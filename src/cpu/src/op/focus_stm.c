...
uint8_t write_focus_stm(const volatile uint8_t* p_data) {
  ...
  const uint8_t segment = (p->head.flag & FOCUS_STM_FLAG_SEGMENT) != 0 ? 1 : 0;
    ...
    switch (segment) {
      case 0:
        ...
        bram_cpy(BRAM_SELECT_CONTROLLER, ADDR_STM_SOUND_SPEED0_0,
                 (uint16_t*)&p->head.sound_speed, sizeof(uint32_t) >> 1);
  ...
  if (size <= page_capacity) {
    bram_cpy_focus_stm(
        (_stm_cycle[segment] & FOCUS_STM_BUF_PAGE_SIZE_MASK) << 2, src, size);
    _stm_cycle[segment] = _stm_cycle[segment] + size;
  } else {
    bram_cpy_focus_stm(
        (_stm_cycle[segment] & FOCUS_STM_BUF_PAGE_SIZE_MASK) << 2, src,
        page_capacity);
    ...
    bram_cpy_focus_stm((_stm_cycle[segment] & FOCUS_STM_BUF_PAGE_SIZE_MASK)
                           << 2,
